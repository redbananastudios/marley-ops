...
export default async function AiSurveyReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: rooms }, { data: detections }, { data: mediaRows }, { data: segmentRows }] = await Promise.all([
    admin.from("cubic_survey_rooms").select("id, name, status, coverage").eq("survey_id", survey.id).order("sort"),
    admin.from("cubic_ai_detections").select("id, room_id, segment_id, label, catalogue_key, candidates, qty, confidence, moving, flags, evidence, review_reason, state").eq("survey_id", survey.id).order("created_at"),
  ...
  const reviewDetections: ReviewDetection[] = (detections ?? []).map((row) => ({
    id: row.id, roomId: row.room_id, segmentId: row.segment_id, label: row.label, catalogueKey: row.catalogue_key,
  ...
  return <main className="w-full px-4 pb-10 md:px-7">
    ...
    <ReviewWorkspace surveyId={survey.id} leadId={leadId} initialUpdatedAt={survey.updated_at} manifestComplete={survey.room_manifest_complete} rooms={rooms ?? []} media={media} detections={reviewDetections} catalogue={catalogue} initialSegments={segments} />
  </main>;
}
