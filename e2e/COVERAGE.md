...
| Reopening a declined lead clears the stale `leads.deposit_amount`/`deposit_requested_at`/`balance_amount`/`balance_due_date` denormalised copy the Payments card reads (PRs #226/#227), alongside the pre-existing `quotes`-row voided-invoice clear | ✅ | office/lead-reopen-clears-stale-balance.spec.ts — live-verified against staging by the 2026-09-04 QA audit: seeded a declined lead+quote in exactly the pre-reopen state (stale £150 deposit/£650 balance on `leads`, `quotes.status='accepted'` with voided Zoho invoice ids and `booking_cancelled_at` set), clicked the real "Reopen" button, confirmed the Payments card no longer showed the stale figures and SQL confirmed both the `leads` and `quotes` clears fired (and that `deposit_paid_at`/`balance_paid_at` were never touched). Passed 2/2 against deployed staging before shipping. 0 findings |
| Mark lost on an accepted quote — deposit invoice voided against the STAMPED provider (`asProvider(deposit_invoice_provider)`), success evidenced by the `activities` "voided" note, not just a status flip | 🟡 | office/ledger-void-routing.spec.ts — closes `admin_ledger_void_routing_spec` (qa/state.json, open since 2026-08-27, previously only code+SQL spot-checked). Live-verified end to end against staging by the 2026-09-04 QA audit's admin role-agent: real `/q/<token>` bank-transfer accept raised a genuine staging-Zoho deposit invoice (`deposit_invoice_provider='zoho'` stamped at write time), a real "Mark lost" click produced the toast "Marked lost. 1 unpaid invoice voided in Zoho." and the expected `activities` "voided" note (not the catch block's silent ops-alert-only failure path) — 0 findings, provider-routing invariant holds (0 mismatches across 19 live staging quotes). This file passed 2/2 against deployed staging before shipping. `test.skip`'d: this env has no `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` so `E2E_DB_READY` is false, same standing gap as every other DB-seeded spec, not blocked by any bug |
...
| Deposit received (bank transfer) writes a genuine customer-facing dry-run comms row, distinct from the internal ops alert | ✅ | office/deposit-comms-dryrun.spec.ts — proven live against staging 2026-09-04. Its original row locator (QA-20260904-01: ambiguous `hasText`+`.last()` resolved to a text-only inner div with no action button) is fixed and re-verified passing end to end |
| Same action for a Pitmans-brand quote: `brandForComms` resolves without error, comm is correctly scoped | ✅ | office/deposit-comms-dryrun-pitmans.spec.ts — added 2026-09-04, closing the multi-brand PRD §13 gap that no e2e spec drove a real (non-synthetic-fixture, non-standalone-script) Pitmans send. `communications.body` persists only the brand-agnostic plain-text summary (see file docstring) so this proves the pipe doesn't break for a non-default brand, not HTML branding — that's covered by tests/lib/comms/email-brand.test.ts + the manual visual check against the live Resend template push |
| Move-date confirmation ("Confirm in person") writes BOTH a customer-facing dry-run email AND SMS, not email only | ✅ | office/date-confirm-comms-dryrun.spec.ts — added 2026-09-04 (Peter: "implement the removal booking SMS for confirmed dates"). Closes a gap that had NO e2e coverage at all before this — a removal's date-confirmation step only ever emailed; the SMS builder (`dateConfirmationSms`, lib/comms/date-confirm-email.ts) mirrors the survey rail's booked/moved/cancelled notices, which already send both channels. Seeded so the deposit already covers the 25% commitment (`amount <= 0` in `ensureCommitmentInvoice`), so this proves the wiring through the real `confirmDateInPersonAction` → `confirmMoveDate` → `sendDateConfirmationEmail` path with ZERO Zoho API calls, deliberately — see the file docstring re: the shared staging org's daily quota (QA-20260902-06) |
...
| IO proof: office "Deposit received" (bank transfer) writes a genuine customer-facing dry-run `communications` row, distinct from the internal ops-alert email a customer's own `/q/<token>` actions trigger | 🟡 | office/deposit-comms-dryrun.spec.ts — added 2026-09-04 QA audit, closing `io.email_sms_dispatch_dryrun` (stale since 2026-08-20, needed a paired customer+admin handoff a solo role-agent couldn't reach). Proven live against staging by the audit's Sonnet role-agent: real `/q/<token>` accept, real throwaway admin login, real `/bookings` "Deposit received → Bank transfer" click, `communications` row read back with `provider_id` starting `dryrun-email-`, right recipient/amount/quote-ref, matching what the dialog said and what the customer's own reload of `/q/<token>` showed — 0 findings, before this spec was written from that recipe. `test.skip`'d: this env has no `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` so `E2E_DB_READY` is false — the file itself has never executed via the real Playwright runner here, same standing gap as every other DB-seeded spec, not blocked by any bug |
...
| Doorstep contract dialog's terms link matches the JOB's brand, not always the default's (PR #228) | ✅ | crew/doorstep-contract-brand.spec.ts — added 2026-09-04 QA audit (freshest code: components/crew/collect-contract-button.tsx + app/my-jobs/[id]/page.tsx changed the same day). `brands.pitmans.terms_url` is NULL by default, which would make either version of the code render the same fallback URL, so the spec temporarily gives Pitmans a marker `terms_url` (Settings > Brands' safe "Terms link" field) for the run and restores it in `afterAll`; asserts a Marley-brand job as a negative control (still the literal default). Live-verified 2/2 against deployed staging this run (own throwaway crew+office+estimator logins substituted for the persistent fixtures via env override, since this run's shell had no `E2E_*_PASSWORD`) before shipping unskipped |
...
| /cv customer photo upload (QA-20260827-04): attach a photo through the token link, no login — DB shows a `customer_uploaded=true` row pointing at a real, byte-identical bucket object | ✅ | public/cv-photo-upload.spec.ts — added 2026-09-03 QA audit, reusing the seed/upload/read-back shape the finding's closing verification proved live against staging. tsc + eslint clean, `playwright test --list` loads both tests under `public`; the underlying HTTP round-trip (seed → real multipart POST to `/cv/<token>/photos` → service-role read-back of `survey_photos` + a byte-identical bucket download → full teardown incl. the `activities` row the route's first-photo timeline note writes) was validated live against staging via a byte-identical scratch script before this file was written. `test.skip`'d: this env has no `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` (only `QA_STAGING_*`), so `E2E_DB_READY` is false and the file itself has never executed via the real Playwright runner here — same standing gap as every other DB-seeded spec in this suite, not a bug |
