...
/* ------------------------------------------------------------- availability
 * Both live in `card-availability.ts` now — a leaf module that imports nothing
 * from this file, so the COMMS layer can ask the same question without closing
 * an import cycle through the accept flow. Re-exported here because every
 * existing importer names this module, and because there must stay exactly ONE
 * implementation of the three-way AND. */
export { cardPaymentsAvailable, cardEnabledBrands } from "@/lib/payments/card-availability";
