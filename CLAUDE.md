...
Last touched: 2026-07-28 on i9 — **CARD PAYMENTS hardened + refunds tested end-to-end + a VAT guard-rail on refunds; one go-live blocker surfaced (takepayments IP allowlist).**
- **Adversarial review of the reconcile/redirect fixes → 5 defects fixed** (`lib/payments/card-payments.ts`): whitelist redirect so ONLY a genuine `paid` race-loser shows success (a declined/superseded/stale-read attempt no longer lands on `card=ok`); reconcile escalation now covers charged-but-`abandoned` rows (two-tab hole); the dead 24h abandon sweep made reachable (7-day fetch window); oldest-first ordering on both reconcile sweeps; the money-critical redirect table + escalation now unit-tested.
- **Refunds tested end-to-end vs the takepayments SANDBOX** (`tests/integration/card-refund-sandbox.test.ts`, opt-in `RUN_SANDBOX_REFUND=1`): a Direct SALE (`threeDSRequired=N`) mints refundable xrefs; REFUND_SALE partial→full + audit rows PROVEN; over-refund guard proven. **GO-LIVE BLOCKER:** takepayments IP-restricts the Direct-API money-out/management actions (REFUND_SALE, CANCEL, reconcile QUERY) → rc 65558 "IP blocked primary" from an unregistered IP. Box IP `51.195.253.165` must be allowlisted in the MMS or live refunds + reconcile fail (go-live checklist **C7**); CANCEL/void + race + rollback scenarios skip until then.
- **VAT credit-note guard-rail SHIPPED** (`raiseCreditNoteReminder`; Peter chose guard-rail over full automation): a card refund/void now raises a tracked `follow_ups` task + accounts@ alert to reverse the output VAT via a MANUAL Zoho credit note (forfeited/retained deposits keep their VAT, unchanged). Pure `buildCreditNoteReminder` unit-tested. Full Zoho-credit-note automation is the deferred fast-follow (checklist **C8**, accountant sign-off).
- **Verified:** tsc 0 / lint 0 err / **1148 vitest** / build green. Detail in memory [[marley-takepayments]] + go-live checklist C7/C8.
