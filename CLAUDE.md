...
- (08-06) **`e18af70` — bank-feed attribution:** commitment invoices are a first-class match kind (MY SAFETY LTD £50 → MMR034 one-tap, confirmed by the office 08-07 — first real case) + Attach-to-quote dialog on unmatched/mismatch rows (exact-amount server-enforced) + paid commitments in the Received view + migration 0086 + 9 review findings fixed (headline: balance nets out a RAISED commitment). **`41292d8`** — /payments refresh button (runs the bank sync then re-renders). AI-suggestion layer deliberately deferred → [ClickUp 869ef5wdu].
