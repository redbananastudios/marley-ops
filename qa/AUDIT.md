...
- Fifth sandbox quirk (found 2026-08-20, broke CI on the very push that reported it): triggering `/api/cron/health-watchdog` for the "IO proof" cron check is NOT read-only — it writes real rows into `operational_issues` for every stale/never-run job it finds (`reportOperationalIssue`), and since staging has no live cron scheduler this is likely the FIRST time each of those ~13 alerts has ever fired, so it surfaces brand-new "X has never run" banners on `/automations` that didn't exist before your run. One of those collided with `e2e/office/journey.spec.ts`'s `getByText("Crew job sheets")` (not `exact: true`) via Playwright's strict-mode duplicate-match check, failing the very next CI run. If you trigger health-watchdog, immediately resolve the `operational_issues` rows it created (`issue_key ilike 'watchdog:%'`, `status='open'`, `first_seen_at` matching your call) as part of cleanup — same principle as any other marker row, just a different table — rather than leaving new alert noise for the real business owners' daily digest (which is dry-run/simulated on staging, but don't rely on that).
...
4. **The rota.** Read `qa/state.json`, pick the stalest items per role list — but first, anything whose source files changed since the ledger's `lastAuditSha` jumps the queue (freshest code is likeliest broken). Sizing per run: ~a dozen operations across the four role agents + 1–2 handoff scenarios + IO/truth items + 1–2 permanent specs. Dispatch one subagent per role (run them in parallel; keep judgment, spec-writing and fix-writing in the main loop). **Role agents run on `sonnet`, never `haiku`.** The 2026-08-24T04:32Z run dispatched all four on Haiku and every one of them reported live UI verification it had not performed — one discovered column names by trial and error against the service-role client, which proves it went round the server actions rather than through them — and two explicitly claimed a clean teardown while leaving 7 orphaned auth accounts behind. Nothing they reported was usable, so the run cost a full dispatch and produced no verification. "Cheaper for mechanical driving" was the wrong read: this work is not mechanical, it is *doing a thing and honestly reporting whether you did it*.

   Because a confident narrator is not a tier-specific failure, a role agent's report is only evidence if it carries **literal automation artifacts**: the `page.url()` trail it actually visited, and the SQL read-back it actually ran. Prose describing what a flow "would" do, or what an existing spec covers, is not evidence — discard it and do not update `qa/state.json` from it. The main loop **spot-checks one claim per agent every run** (independently re-querying, not re-reading the report), including a full `auth.users` sweep, because self-reported cleanup was the claim that failed first.
