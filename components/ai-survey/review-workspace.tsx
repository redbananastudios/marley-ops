  ...
  return (
    <div>
      ...
      <aside className="rounded-2xl border border-border bg-card p-4">
        ...
        {!!manualConflicts.length && <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="text-sm font-bold text-amber-950">Already in the manual inventory?</p><div className="mt-2 space-y-2">{manualConflicts.map((conflict) => <div key={conflict.detectionId} className="rounded-lg bg-white p-2"><p className="text-xs font-semibold">AI: {conflict.detectionLabel} · Manual: {conflict.existingTitle}</p><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" onClick={() => setManualChoices((current) => ({ ...current, [conflict.detectionId]: "skip_ai" }))} className={cn("focus-ring min-h-10 rounded-lg border text-xs font-bold", manualChoices[conflict.detectionId] === "skip_ai" && "border-mm-red bg-mm-red text-white")}>Already counted</button><button type="button" onClick={() => setManualChoices((current) => ({ ...current, [conflict.detectionId]: "separate" }))} className={cn("focus-ring min-h-10 rounded-lg border text-xs font-bold", manualChoices[conflict.detectionId] === "separate" && "border-mm-red bg-mm-red text-white")}>Separate item</button></div></div>)}</div></div>}
      ...
      <div className="sticky bottom-3 z-20 mt-4 flex flex-wrap items-center gap-3 rounded-2xl border border-border bg-card/95 px-4 py-3 shadow-lg backdrop-blur"><div><p className="text-xs font-bold uppercase tracking-wide text-mist-400">{planningReady ? "Planning volume" : "Provisional inventory"}</p><p className="text-sm font-semibold">{planningReady ? <>Raw {initialRawFt3.toLocaleString("en-GB")} ft³ · +{contingencyPct}% · Planning {planningFt3.toLocaleString("en-GB")} ft³</> : <>Provisional raw {provisionalRawFt3.toLocaleString("en-GB")} ft³ · complete all rooms for vehicle guidance</>}</p></div>{recommendation && <span className="ml-auto inline-flex min-h-10 items-center gap-2 rounded-pill bg-mm-red px-3 text-sm font-bold text-white"><Truck className="size-4" /> {vehicleShortLabel(recommendation)}</span>}</div>
    </div>
  );
}
