...
interface ProblemJob { id: string; status: string; attempts: number; max_attempts: number; error: string | null; created_at: string }

export function AiSettingsCard({ settings, spentUsd, mediaBytes, configured, canEdit, problemJobs }: { settings: BusinessSettings; spentUsd: number; mediaBytes: number; configured: boolean; canEdit: boolean; problemJobs: ProblemJob[] }) {
  ...
  function retryJob(jobId: string) {
    startTransition(async () => {
      const result = await retryAiJobAction(jobId);
      if (result.ok) { toast.success("AI job queued again."); router.refresh(); }
      else toast.error(result.error);
    });
  }
